...
class VariantCalling:
        ...
        for varscan_type in ("snp", "indel"):
            ...
            if len(filenames) > 0:
                    ...
                    for record in vcf.Reader(filename=varscan_filename):
                        for sample in record.samples:
                            sample_data = {
                                ...
                                "indelError":
                                "indelError" not in record.FILTER,
                                "QUAL": record.QUAL,
                                "GT": sample.data.GT,
                                "GQ": sample.data.GQ,
                                "RD": sample.data.RD,
                ...
                current_data = pd.DataFrame(
                    ...
                    columns=[
                        "chr", "start", "end", "width", "ref", "alt",
                        "totalDepth", "refDepth", "altDepth", "sampleNames",
                        "indelError", "QUAL", "DP", "SOMATIC", "SS", "SSC",
                        "GPV", "SPV", "GT", "GQ", "RD", "FREQ", "DP4", "ADF",
                        "ADR"
                    ])
                ...
                if withNormals:
                    ...
                    normals = normal.merge(
                        tumor, on="key",
                        how="left")[["totalDepth", "refDepth", "altDepth"]]
        ...
        if os.path.exists(self.strelka_results_dir):
            ...
            for strelka_type in ("snvs", "indels"):
                passed_vcf_filename = os.path.join(
                    self.strelka_results_dir,
                    "passed.somatic.%s.vcf" % strelka_type)
                    ...
                    for alternative_base in record.ALT:
                        ...
                        current_data = {
                            ...
                            (record.CHROM, record.POS,
                             record.POS + len(record.REF) - 1, record.REF,
                             alternative_base),
                            "DP":
                            coverage,
                            "FREQ":
                            alternative_coverage / total_coverage
                        }
    ...
    def collect_annotated_variants(self):
        ...
        annotation["gene_type"] = annotation.merge(
            gene_info, left_on="Gene.refGene", right_on="symbol",
            how="left").cancer_type.values
        ...
        annotation.loc[annotation["Func.refGene"] == "splicing",
                       "AAChange.refGene"] = annotation[annotation[
                           "Func.refGene"] == "splicing"]["GeneDetail.refGene"]
        annotation.loc[annotation["Func.refGene"] == "UTR3",
                       "AAChange.refGene"] = annotation[annotation[
                           "Func.refGene"] == "UTR3"]["GeneDetail.refGene"]
        annotation.loc[annotation["Func.refGene"] == "UTR5",
                       "AAChange.refGene"] = annotation[annotation[
                           "Func.refGene"] == "UTR5"]["GeneDetail.refGene"]

        annotation["Gene.refGene"] = annotation["Gene.refGene"].map(
            lambda value: re.split(R"[^A-Za-z0-9]", value)[0])

        annotation.loc[annotation.CADD13_PHRED > VariantCalling.medium_damage,
                       "damaging"] = "Medium"
        ...
        annotation.loc[
            empty_canonical_refseqs, "alternative_refseq"] = annotation.loc[
                empty_canonical_refseqs, "AAChange.refGene"].tolist()
        annotation.loc[empty_canonical_refseqs,
                       "hgnc_canonical_refseq"] = float("nan")
        ...
        annotation.loc[set([index[0]
                            for index in overlaps]), "in_gene_panel"] = True
        ...
        annotation_all.to_csv(
            os.path.join(self.analysis.get_out_dir(),
                         self.analysis.basename + "_variants.csv"),
            index=False)
        annotation_all[[
        ...
        if config.use_mongodb:
            ...
            barcoded_sample = BarcodedFilename.from_sample(
                self.analysis.sample)

            self.variants = pd.read_csv(
                self.variants_filename, index_col=False)
            variants = [{
                key.replace(".", " "): value
            ...
            annotation_ids = [
                find_or_insert(db.annotations, {"id": annotation["id"]},
                               annotation)["_id"] for annotation in annotations
            ]
